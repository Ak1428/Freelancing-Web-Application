 ...
 * PROTECTED: Only the freelancer themselves or admin can view their job suggestions
 */
export async function GET(req: Request) {
  try {
    ...
    // Authorization: user can only view suggestions for themselves, or admin can view all
    const isOwnProfile = freelancerId === (session.user as any).id;
    const isAdmin = (session.user as any).role === 'ADMIN';

    if (!isOwnProfile && !isAdmin) {
      return NextResponse.json(
        { error: 'Forbidden. You can only view your own job suggestions.' },
        { status: 403 }
      );
    }
