 ...
 * Get freelancer suggestions for a specific job
 * PROTECTED: Only the job owner (client) can view suggestions for their job
 */
export async function GET(req: Request) {
  try {
    ...
    // Authorization: only the job owner or admin can view suggestions
    const isJobOwner = job.clientId === (session.user as any).id;
    const isAdmin = (session.user as any).role === 'ADMIN';

    if (!isJobOwner && !isAdmin) {
      return NextResponse.json(
        { error: 'Forbidden. You can only view suggestions for your own jobs.' },
        { status: 403 }
      );
    }
